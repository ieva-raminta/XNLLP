...
parser.add_argument('--model', type=str, default="longformer", required=False)
...
if args.model == "longformer":
    ...
    model_directory_path = "/home/irs38/Negative-Precedent-in-Legal-Outcome-Prediction/results/Outcome/joint_model/legal_bert/facts/ee0be937361c453182b7eee073322591/"
    MODEL_NAME="nlpaueb/legal-bert-base-uncased"
    tokenized_dir = "/home/irs38/Negative-Precedent-in-Legal-Outcome-Prediction/ECHR/Outcome/legal_bert"
elif args.model == "longformer_binary": 
    model_directory_path = "/home/irs38/Negative-Precedent-in-Legal-Outcome-Prediction/results/Outcome/baseline_positive/longformer/facts/a6da228e9acd4d269a13305565cedeae/"
    MODEL_NAME="allenai/longformer-base-4096"
    tokenized_dir = "/home/irs38/Negative-Precedent-in-Legal-Outcome-Prediction/ECHR/Outcome/longformer"
elif args.model == "legal_bert_binary":
    model_directory_path = "/home/irs38/Negative-Precedent-in-Legal-Outcome-Prediction/results/Outcome/baseline_positive/legal_bert/facts/770340609bf546908467bdab14f8df5b/"
    MODEL_NAME="nlpaueb/legal-bert-base-uncased"
    tokenized_dir = "/home/irs38/Negative-Precedent-in-Legal-Outcome-Prediction/ECHR/Outcome/legal_bert"
...
def make_loader(input, mask, labels, claims, train=True):
    ...
    claims = torch.LongTensor(claims)
...
def predict(b_input_ids, b_attn_mask=None, global_attention_mask=None, b_claims=None):
    logits, _ = model(b_input_ids.cuda(), b_attn_mask.cuda(), global_attention_mask, b_claims)
    return logits
...
def forward_func(b_input_ids, b_attn_mask=None, global_attention_mask=None, b_claims=None, article_id=0):
    ...
    if "binary" in args.model: 
        logits = logits.reshape(b_input_ids.shape[0], -1, 2)
    else: 
...
ids_to_silver_rationales = {}
for id,rationale in zip(ids, silver_rationales):
    ids_to_silver_rationales[id] = rationale
ids_to_ex = {}
...
for i,item in enumerate(dev_data): 
    ...
    rationale = ids_to_silver_rationales[gold_id] # flag for silver vs gold rationales: ids_to_silver_rationales vs ids_to_gold_rationales
    ...
    if rationale not in ["[]", []]: 
        ...
        for label in [0,1,2]:
            ...
            if label in out: 
                ...
                num_sentences_until_max_tokens = 0
                accummulated_sentence_lengths = 0

                for i,sentence_length in enumerate(sentence_lengths):
                    if accummulated_sentence_lengths + sentence_length > max_len:
                        attr_per_sentence.append(0)
                        attr_summary = attr_summary[sentence_length:]
                        accummulated_sentence_lengths += sentence_length
                    else: 
                        attr_per_sentence.append(attr_summary[:sentence_length].sum().item())
                        attr_summary = attr_summary[sentence_length:]
                        accummulated_sentence_lengths += sentence_length
                        num_sentences_until_max_tokens += 1
