...
class BertClassifier(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, global_attention, claims):
        ...
        if self.longformer:
            max_len = 4096
            outputs = self.model(input_ids=input_ids[:,:max_len], attention_mask=attention_mask[:,:max_len], global_attention_mask=global_attention[:,:max_len], output_hidden_states=True)
        else:
            max_len = 512
            outputs = self.model(input_ids[:,:max_len], attention_mask=attention_mask[:,:max_len], output_hidden_states=True)
